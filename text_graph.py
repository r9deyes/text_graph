...
class text_graph(nx.DiGraph):
    path='.\mystem-3.0-win7-32bit'
    node_list=None
    node_property=None
    edge_list=None
    edge_property=None

    def __init__(s,text,fromFile=0,stop_words=('-'),**kwargs):
        nx.DiGraph.__init__(s)
        if (fromFile):
            with open(text) as f:
                text=f.read()
        if kwargs.get('word_pattern'):
            word_pattern = kwargs['word_pattern']
        else:
            word_pattern = None
        try:
        ...
        if (word_pattern):
            ss = text_words(text, stop_words=stop_words, words_pattern=word_pattern)  # .decode(encoding))#text.decode('cp1251').split()#
        else:
            ss = text_words(text, stop_words=stop_words)  # .decode(encoding))#text.decode('cp1251').split()#
...
class sentence_graph(text_graph):
    def __init__(self, text, fromFile=0, stop_words=('-')):
        ...
        for v in self.node.keys():
            ...
            self.node[v]['PositionFirstSent'] /= float(self.node[v]['sentCount'])
            self.node[v]['PositionLastSent'] /= float(self.node[v]['sentCount'])
            self.node[v]['PositionAvgSent'] /= float(self.node[v]['weight'])
    ...
    def compute_stats_text_words(self, wrd, sn, glbPos=0):
        ...
        for tt in wrd:
            ...
            if  wrd[t] not in sentence_graph[sn]:
                if not(self.node[wrd[t]].get('PositionFirstSent')):
                    self.node[wrd[t]][		'PositionFirstSent'] = 	t + 1
                else:
                    self.node[wrd[t]][		'PositionFirstSent'] += t + 1
                self.node[wrd[t]][			'PositionLastSent'] = 	0
                self._add_node(wrd[t],		'sentCount')
